...
async function latestStoriesReportPath(): Promise<string> {
  const allNames = await readdir(outputDirectory);
  const matches = allNames
    .filter(
      (name) =>
        name.startsWith('instagram-stories-pilot-') && name.endsWith('.json')
    )
    .sort()
    .reverse();
  const latest = matches[0];
  if (!latest) {
    throw new Error('No Instagram Stories report found.');
  }
...
function buildReviewHtml(report: StoriesReport): string {
...
<head>
  ...
  <title>Pulso Scout Stories — Revue locale</title>
  <style>
...
<body>
  <header>
    <div><div class="eyebrow">Pulso Scout — Stories</div><h1>Revue locale des Stories analysées par IA</h1></div>
  ...
  <script>
    ...
    const storageKey = 'pulso-scout-stories-review:' + data.generatedAt;
    const outcomes = [
    ...
    function render() {
      ...
      document.getElementById('cards').innerHTML = visible.length ? visible.map(candidate => {
        ...
        return '<article data-id="' + escapeHtml(candidate.reviewId) + '">' +
          (candidate.imageUrl ? '<img src="' + escapeHtml(candidate.imageUrl) + '" alt="Story" loading="lazy">' : '<div class="empty">Pas d’image</div>') +
          ...
          '<div class="chips">' + chips + (candidate.analysisError ? '<span class="chip warning">Erreur d’analyse</span>' : '') + '</div>' +
    ...
    document.getElementById('export').addEventListener('click', () => {
      ...
      const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'pulso-scout-stories-decisions.json'; link.click(); URL.revokeObjectURL(link.href);
    });
    render();
  </script>
</body>
...
async function main(): Promise<void> {
  const inputPath = await latestStoriesReportPath();
  const report = JSON.parse(await readFile(inputPath, 'utf8')) as StoriesReport;
  const outputPath = join(outputDirectory, 'instagram-stories-review.html');
