...
Functional Sprint 4 uses the Expo SDK 57-compatible `expo-localization` package to read the Android device language and declare French and English as supported application locales. It uses `@react-native-async-storage/async-storage` only to preserve the manual language choice locally without an account. Both native modules were selected through Expo's supported `expo install` command; no localization, translation, or AI service is used.
